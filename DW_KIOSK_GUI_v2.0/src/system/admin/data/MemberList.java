...
public class MemberList {
	 ...
	 * 파라미터: 없음
	 * 반환값: 없음
	 * 기능 설명: Member 클래스에 지정된 경로에 존재하는 src//files//data//member.txt파일의 내용 전체 출력
	 */
	public void printWholeMember() {
		ArrayList<Member> memberList=new ArrayList<Member>();//Member 객체들을 담을 수 있는 동적 배열을 생성
		BufferedReader in;	// 입력 스트림
		String s;
		String[] tmp;
		
		try {
			in=new BufferedReader(new FileReader(Member.FILE_NAME));
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				Member member = new Member(tmp[Member.INDEX_MEMBER_ID], 
						Integer.parseInt(tmp[Member.INDEX_POINT]), tmp[Member.INDEX_BIRTH]);
	 ...
	 * 반환값: 없음
	 * 기능 설명: Member 클래스에 지정된 경로에 존재하는 src//files//data//member.txt파일의 내용 중
...
//				System.out.println(s);
				...
				if (tmp[Member.INDEX_MEMBER_ID].equals(searchedMemberId)) {//searchedMemberId와 회원ID가 같은 회원 발견
					found=true;
					Member member = new Member(tmp[Member.INDEX_MEMBER_ID], 
							Integer.parseInt(tmp[Member.INDEX_POINT]),tmp[Member.INDEX_BIRTH]);
...
//				System.out.println(s);
				...
				if (tmp[Member.INDEX_BIRTH].equals(searchedBirth)) {//searchedBirth와 생일이 같은 회원 발견
					found=true;
					Member member = new Member(tmp[Member.INDEX_MEMBER_ID], 
							Integer.parseInt(tmp[Member.INDEX_POINT]),tmp[Member.INDEX_BIRTH]);
