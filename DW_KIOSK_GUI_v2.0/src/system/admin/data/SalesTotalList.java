...
public class SalesTotalList{
	...
	public void setFileName(String date) {
		...
		this.date = date;
		fileName = SalesTotal.FILE_DIRECTORY + fileName + "(" + date + ").txt";
	}
	...
	public void printWholeData() {
		...
		try {
			while((s = in.readLine()) != null) {
				...
				totalSalesAmt += Integer.parseInt(tmp[SalesTotal.INDEX_TOTAL_AMT]);
			}
	...
	private String toPrintTemplate(String[] data) {
		...
		str += data[SalesTotal.INDEX_ORDER_TYPE];
		str += "  " + data[SalesTotal.INDEX_WAIT_NO];
		str += "   " + data[SalesTotal.INDEX_TRANS_DT];
		str += "  " + data[SalesTotal.INDEX_ORDER_NO];
		str += "  " + String.format("%,7d", Integer.parseInt(data[SalesTotal.INDEX_TOTAL_AMT]));
		str += "  " + String.format("%,7d", Integer.parseInt(data[SalesTotal.INDEX_SUPPLY_AMT]));
		str += "  " + String.format("%,6d", Integer.parseInt(data[SalesTotal.INDEX_VAT]));
		str += "   " + data[SalesTotal.INDEX_PAY_METHOD];
		str += "  " + String.format("%-15s", data[SalesTotal.INDEX_DISCOUNT_ID]);
		str += "  " + String.format("%-5s", data[SalesTotal.INDEX_CARD_NAME]);
		str += "  " + String.format("%16s", data[SalesTotal.INDEX_CARD_NO]);
		str += "  " + String.format("%2s", data[SalesTotal.INDEX_CARD_QUOTA]);
		str += "     " + String.format("%8s", data[SalesTotal.INDEX_AUTH_CODE]);
		str += "  " + String.format("%11s", data[SalesTotal.INDEX_MEMBER_ID]);
		str += "  " + String.format("%,7d", Integer.parseInt(data[SalesTotal.INDEX_REWARD_PTS]));
		str += "  " + String.format("%,7d", Integer.parseInt(data[SalesTotal.INDEX_TOTAL_PTS]));
	...
	public TreeMap<String, Integer> getRevenueData() {
	    ...
	    for (String[] data : salesTotalList) {
            ...
            String productTransDT = data[SalesTotal.INDEX_TRANS_DT].substring(0,8);
            int salesTotalAmt = Integer.parseInt(data[SalesTotal.INDEX_TOTAL_AMT]);
