...
public class CouponList {
	 ...
	 * 파라미터: 없음
	 * 반환값: 없음
	 * 기능 설명: Coupon 클래스에 지정된 경로에 존재하는
	 * 			src//files//data//coupon.txt파일의 내용 전체 출력
	 */
	public void printWholeCoupon() {
		ArrayList<Coupon> couponList = new ArrayList<Coupon>();// Coupon 객체들을 담을 수 있는 동적 배열을 생성
		BufferedReader in; // 입력 스트림
		String s;
		String[] tmp;

		try {
			in = new BufferedReader(new FileReader(Coupon.FILE_NAME));
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				Coupon coupon = new Coupon(tmp[Coupon.INDEX_COUPON_ID],
						Integer.parseInt(tmp[Coupon.INDEX_PRICE]), tmp[Coupon.INDEX_USED]);
	 ...
	 * 반환값: 없음
	 * 기능 설명: Coupon 클래스에 지정된 경로에 존재하는 src//files//data//coupon.txt파일의 내용 중 searchedId와
	 * Coupon.INDEX_COUPON_ID가 동일한 쿠폰 목록 출력
	 */
	public void searchCouponById(String searchedId) {
		ArrayList<Coupon> couponList = new ArrayList<Coupon>();// Coupon 객체들을 담을 수 있는 동적 배열을 생성
		BufferedReader in; // 입력 스트림
		boolean found = false;// searchedId와 Coupon.INDEX_COUPON_ID가 동일한 쿠폰 목록을 발견했는지 확인할 변수
		String s;
		String[] tmp;
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				if (tmp[Coupon.INDEX_COUPON_ID].equals(searchedId)) {// searchedId와 Coupon.INDEX_COUPON_ID가
																			// 동일한 쿠폰 목록을 발견
					found = true;
					Coupon coupon = new Coupon(tmp[Coupon.INDEX_COUPON_ID],
							Integer.parseInt(tmp[Coupon.INDEX_PRICE]), tmp[Coupon.INDEX_USED]);
			...
			in.close();
			if (found == false) {// searchedId와 Coupon.INDEX_COUPON_ID가 동일한 쿠폰 목록을 발견하지 못하면
		...
		if (found == true) {// searchedId와 Coupon.INDEX_COUPON_ID가 동일한 쿠폰 목록을 발견했으면 출력 시작
	 ...
	 * 반환값: 없음
	 * 기능 설명: Coupon클래스에 지정된 경로에 존재하는 src//files//datas//coupon.txt파일의 내용 중 
	 * 			searchedPrice와 Coupon.INDEX_PRICE가 동일한 쿠폰 목록 출력
	 */
	public void searchCouponByPrice(int searchedPrice) {
		ArrayList<Coupon> couponList = new ArrayList<Coupon>();// Coupon 객체들을 담을 수 있는 동적 배열을 생성
		BufferedReader in; // 입력 스트림
		boolean found = false;// searchedPrice와 Coupon.INDEX_PRICE가 동일한 쿠폰 목록을 발견했는지 확인할 변수
		String s;
		String[] tmp;
...
//				System.out.println(s);
				...
				int PriceInt = Integer.parseInt(tmp[Coupon.INDEX_PRICE]);

				if (searchedPrice == PriceInt) {// searchedPrice와 Coupon.INDEX_PRICE가 동일한 쿠폰 목록을 발견
					found = true;
					Coupon coupon = new Coupon(tmp[Coupon.INDEX_COUPON_ID],
							Integer.parseInt(tmp[Coupon.INDEX_PRICE]), tmp[Coupon.INDEX_USED]);
			...
			in.close();
			if (found == false) {// searchedPrice와 Coupon.INDEX_PRICE가 동일한 쿠폰 목록을 발견하지 못하면
		...
		if (found == true) {// searchedPrice와 Coupon.INDEX_PRICE가 동일한 쿠폰 목록을 발견했으면 출력 시작
	 ...
	 * 							searchedUsed와 Coupon.INDEX_USED가 동일한 쿠폰 목록 출력
	 */
	public void searchCouponByUsed(String searchedUsed) {
		ArrayList<Coupon> couponList = new ArrayList<Coupon>();// Coupon 객체들을 담을 수 있는 동적 배열을 생성
		BufferedReader in; // 입력 스트림
		boolean found = false;// searchedUsed와 Coupon.INDEX_USED가 동일한 쿠폰 목록 발견했는지 확인할 변수
		String s;
		String[] tmp;
...
//				System.out.println(s);
				...
				if (tmp[Coupon.INDEX_USED].equals(searchedUsed)) {// searchedUsed와 Coupon.INDEX_USED가 동일한 쿠폰 목록 발견
					found = true;
					Coupon coupon = new Coupon(tmp[Coupon.INDEX_COUPON_ID],
							Integer.parseInt(tmp[Coupon.INDEX_PRICE]), tmp[Coupon.INDEX_USED]);
			...
			in.close();
			if (found == false) {// searchedUsed와 Coupon.INDEX_USED가 동일한 쿠폰 목록 발견하지 못하면
		...
		if (found == true) {// searchedUsed와 Coupon.INDEX_USED가 동일한 쿠폰 목록 발견했으면 출력 시작
	 ...
	 * 반환값: 없음
	 * 기능 설명: Coupon 클래스에 지정된 경로에 존재하는 src//files//data//coupon.txt파일에 입력한 내용을 추가한다.
	 */
	// 카테고리별로 마지막 쿠폰 순번을 찾는 메소드
	private int findLastCouponNo(String temp_member_id) {
		int lastCouponNo = 0;
		
		try (BufferedReader in = new BufferedReader(new FileReader(Coupon.FILE_NAME))) {
			String s;
			String[] tmp;
			...
			while ((s = in.readLine()) != null) {
				tmp = s.split("\\|");
				try {
					String substring = tmp[Coupon.INDEX_COUPON_ID].substring(12, 15);
					if (temp_member_id.equals(tmp[Coupon.INDEX_COUPON_ID].substring(1, 12)))
...
//				System.out.println(substring);
	...
	public void addCoupon() {
		...
		newMenuData[Coupon.INDEX_COUPON_ID] = "C" + temp_member_id + String.format("%03d", newCouponNo);
		...
		newMenuData[Coupon.INDEX_PRICE] = Integer.toString(addCouponPrice);
		newMenuData[Coupon.INDEX_USED] = "Y";
		...
		try {
			in = new BufferedReader(new FileReader(Coupon.FILE_NAME));
			inMember=new BufferedReader(new FileReader(Member.FILE_NAME));
			out = new PrintWriter(new FileWriter(Coupon.TMP_FILE_NAME));
		} catch (FileNotFoundException e) {
			// TODO: handle exception
		...
		try {	
			while ((memberString=inMember.readLine())!=null) {
				...
				if (members[Member.INDEX_MEMBER_ID].equals(temp_member_id)) {
					found = true;
				}
			}
			...
			while ((s = in.readLine()) != null) {
				...
				if (tmp[Coupon.INDEX_COUPON_ID].substring(1,12).equals(temp_member_id)) {
					found = true;
				} 	
				out.println(s);		
			}
