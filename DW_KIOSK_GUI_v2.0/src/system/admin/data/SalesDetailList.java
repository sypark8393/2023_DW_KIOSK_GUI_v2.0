...
public class SalesDetailList {
	...
	public void setFileName(String date) {
		...
		this.date = date;
		fileName = SalesDetail.FILE_DIRECTORY + fileName + "(" + date + ").txt";
	}
	...
	public void searchData(int index, String keyword) {
		...
		if(index == SalesDetail.INDEX_TYPE) {
	...
	private String toPrintTemplate(String[] data) {
		...
		str += data[SalesDetail.INDEX_ORDER_NO];
		str += "  " + data[SalesDetail.INDEX_TYPE];
		str += "   " + String.format("%-15s", data[SalesDetail.INDEX_ID]);
		str += "  " + String.format("%-30s", data[SalesDetail.INDEX_NAME]);
		str += "\t" + String.format("%,5d", Integer.parseInt(data[SalesDetail.INDEX_PRICE]));
		str += "  " + String.format("%3d", Integer.parseInt(data[SalesDetail.INDEX_QUANTITY]));
		str += "  " + String.format("%,7d", Integer.parseInt(data[SalesDetail.INDEX_TOTAL_PRICE]));
	...
	*/
    public HashMap<String, Integer> getProductSalesData() {
        ...
        for (String[] data : salesDetailList) {
            if (!data[SalesDetail.INDEX_NAME].equals("포인트 사용") && !data[SalesDetail.INDEX_NAME].equals("쿠폰 할인")) {//상품명이 포인트 사용이나 쿠폰 할인인 경우는 상품이 팔린 경우가 아니기 때문에 제외
                String menuName=data[SalesDetail.INDEX_NAME];
                int quantity=Integer.parseInt(data[SalesDetail.INDEX_QUANTITY]);//판매 개수 가져오기
	...
	public HashMap<String, Integer> getRevenueData() {
	    ...
	    HashMap<String, Integer> productSalesData = new HashMap<>();
	    for (String[] data : salesDetailList) {
	        if (!data[SalesDetail.INDEX_NAME].equals("포인트 사용") && !data[SalesDetail.INDEX_NAME].equals("쿠폰 할인")) {
	            String productName = data[SalesDetail.INDEX_NAME];
	            int salesAmount = Integer.parseInt(data[SalesDetail.INDEX_TOTAL_PRICE]);
