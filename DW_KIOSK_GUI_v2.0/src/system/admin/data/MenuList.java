...
public class MenuList {
	 ...
	 * 파라미터: 없음
	 * 반환값: 없음
	 * 기능 설명: Menu 클래스에 지정된 경로에 존재하는 src//files//datas//menu.txt파일의 내용 전체 출력
	 */
	public void printWholeMenu() {
		ArrayList<Menu> menuList=new ArrayList<Menu>();//Menu 객체들을 담을 수 있는 동적 배열을 생성
		BufferedReader in;	// 입력 스트림
		String s;
		String[] tmp;
		
		try {
			in=new BufferedReader(new FileReader(Menu.FILE_NAME));
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				//tmp 배열에 저장된 값들을 Menu 열거형의 생성자에 전달
				Menu menu = new Menu(tmp[Menu.INDEX_CATEGORY_ID], tmp[Menu.INDEX_MENU_NO], tmp[Menu.INDEX_NAME],
						Integer.parseInt(tmp[Menu.INDEX_PRICE]), Integer.parseInt(tmp[Menu.INDEX_STOCK]), tmp[Menu.INDEX_ENABLE]);
	...
	public void printWholeCategories() {
		...
		try {
			in=new BufferedReader(new FileReader(Category.FILE_NAME));
		} catch (FileNotFoundException e) {
			// TODO: handle exception
		...
		try {
			while ((s = in.readLine()) != null) {
				tmp = s.split("\\|"); // 읽어온 문자열을 구분자로 분리
				Category category=new Category(tmp[Category.INDEX_CATEGORY_ID], tmp[Category.INDEX_NAME], tmp[Category.INDEX_ENABLE]);
	 ...
	 * 반환값: 없음
	 * 기능 설명: Menu 클래스에 지정된 경로에 존재하는 src//files//datas//menu.txt파일에서 
	...
	public void searchMenuById(String searchedId) {
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				if ((tmp[Menu.INDEX_CATEGORY_ID]+tmp[Menu.INDEX_MENU_NO]).equals(searchedId)) {//searchedId와 메뉴 ID(카테고리 ID+메뉴 순번)가 동일한 메뉴 발견하면
					found=true;
					//Menu 객체 생성
					Menu menu = new Menu(tmp[Menu.INDEX_CATEGORY_ID], tmp[Menu.INDEX_MENU_NO], tmp[Menu.INDEX_NAME],
							Integer.parseInt(tmp[Menu.INDEX_PRICE]), Integer.parseInt(tmp[Menu.INDEX_STOCK]), tmp[Menu.INDEX_ENABLE]);

					menuList.add(menu);//menuList에 위에 생성된 Menu 객체 추가
				}
	...
	public void searchMenuByCategory(String searchedCi) {
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				if (tmp[Menu.INDEX_CATEGORY_ID].equals(searchedCi)) {//카테고리ID가 searchedCi와 같은 메뉴를 발견하면
					found=true;
					Menu menu=new Menu(tmp[Menu.INDEX_CATEGORY_ID], tmp[Menu.INDEX_MENU_NO], tmp[Menu.INDEX_NAME],
							Integer.parseInt(tmp[Menu.INDEX_PRICE]), Integer.parseInt(tmp[Menu.INDEX_STOCK]), tmp[Menu.INDEX_ENABLE]);
	...
	public void searchMenuByStock(int searchedStockMin, int searchedStockMax) {
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				int stockInt = Integer.parseInt(tmp[Menu.INDEX_STOCK]);
	...
	public void editMenuInfo(String editMenuId) {
		...
		try {
		    in = new BufferedReader(new FileReader(Menu.FILE_NAME));
		    out = new PrintWriter(new FileWriter(Menu.TMP_FILE_NAME));
		} catch (FileNotFoundException e) {
		    System.out.println("메뉴 파일을 찾을 수 없습니다.");
		    return;
		} catch (IOException e) {
		...
		try {
		    while ((s = in.readLine()) != null) {//s가 파일을 읽을 수 있는 동안
		        ...
		        if ((tmp[Menu.INDEX_CATEGORY_ID]+tmp[Menu.INDEX_MENU_NO]).equals(editMenuId)) {//editMenuId와 같은 메뉴ID 가진 메뉴 찾으면
		            found = true;
		            //변경할 값 입력받기
		            ...
		            if (!newMenuName.isEmpty() || !newPrice.isEmpty() || !newStock.isEmpty()) {//변경하고자 하는 세 값에 모두 빈 값이 들어간 경우가 아니면
		                ...
		                if (!newMenuName.isEmpty()) {
		                    tmp[Menu.INDEX_NAME]=newMenuName;
		                }
		                if (!newPrice.isEmpty()) {
		                    try {
		                        ...
		                        if (priceUpdate < 0) {
		                            ...
		                            tmp[Menu.INDEX_PRICE] = priceUpdate + ""; //문자열로 출력해주기 위해 ""도 같이 넣기
		                        }
		                ...
		                if (!newStock.isEmpty()) {
		                    try {
		                        ...
		                        if (stockUpdate < 0) {
		                            ...
		                            tmp[Menu.INDEX_STOCK] = stockUpdate + "";//문자열로 출력해주기 위해 ""도 같이 넣기
		                        }
		                    } catch (NumberFormatException e) {
	...
	private int findLastMenuNo(String categoryId) {
	    ...
	    try (BufferedReader in=new BufferedReader(new FileReader(Menu.FILE_NAME))) {
	        String s;
	        String[] tmp;

	        while ((s=in.readLine()) != null) {//파일을 읽어올 수 있는동안
	            tmp=s.split("\\|");
	            if (tmp[Menu.INDEX_CATEGORY_ID].equals(categoryId)) {
	                int menuNo=Integer.parseInt(tmp[Menu.INDEX_MENU_NO]);
	 ...
	 */
	public void addMenu() {
		ArrayList<Category> categoryList=new ArrayList<Category>();	//카테고리 타입 저장할 동적 배열
		...
		while(true) {
			System.out.print(" > ");
			newMenuData[Menu.INDEX_CATEGORY_ID] = sc.nextLine();
			if (Pattern.matches("^[a-zA-Z]+$", newMenuData[Menu.INDEX_CATEGORY_ID])==true) {
				break;
			}else {
	    ...
	    int lastMenuNo=findLastMenuNo(newMenuData[Menu.INDEX_CATEGORY_ID]);
	    int newMenuNo=lastMenuNo+1;
	    newMenuData[Menu.INDEX_MENU_NO]=String.format("%02d", newMenuNo);
		
		//메뉴명 입력
		System.out.println("추가하실 메뉴의 메뉴명을 입력하세요.");
		System.out.print(" > ");
		newMenuData[Menu.INDEX_NAME]=sc.nextLine();
		...
		newMenuData[Menu.INDEX_STOCK]=addMenuStock+"";
		newMenuData[Menu.INDEX_ENABLE]="Y";
		...
		newMenuData[Menu.INDEX_PRICE]=addMenuPrice+"";
		...
		try {
			inMenu=new BufferedReader(new FileReader(Menu.FILE_NAME));
			inCategory=new BufferedReader(new FileReader(Category.FILE_NAME));
			out=new PrintWriter(new FileWriter(Menu.TMP_FILE_NAME));
		} catch (FileNotFoundException e) {
			// TODO: handle exception
			System.out.println("메뉴 파일을 찾을 수 없습니다.");
		...
		try {
			...
			while((sMenu=inMenu.readLine()) != null) {
				...
				for (Category category : categoryList) {
					if (category.getCategoryId().equals(newMenuData[Menu.INDEX_CATEGORY_ID])) {
	...
	public void toggleMenuEnable(String toggleMenuId) {
	    ...
	    try {
	        while (( s=in.readLine()) != null) {
	            ...
	            if ((tmp[Menu.INDEX_CATEGORY_ID]+tmp[Menu.INDEX_MENU_NO]).equals(toggleMenuId)) {
	                found=true;//활성화를 변경할 메뉴를 찾음
	                if (tmp[Menu.INDEX_ENABLE].equals("Y")) {
						tmp[Menu.INDEX_ENABLE]="N"; //활성화 여부를 N으로 변경
					}else tmp[Menu.INDEX_ENABLE]="Y";
	...
	public void addCategory() {
		...
		while(true) {
			System.out.print(" > ");
			newCategoryData[Category.INDEX_CATEGORY_ID]=sc.nextLine();
			if (Pattern.matches("^[a-zA-Z]+$", newCategoryData[Category.INDEX_CATEGORY_ID])==true) {
				break;
			}else {
				System.out.println("카테고리ID는 영어로만 구성되어 있어야 합니다. 다시 입력해주세요.");
			}
		...
		System.out.print(" > ");
		newCategoryData[Category.INDEX_NAME]=sc.nextLine();
		newCategoryData[Category.INDEX_ENABLE]="Y";
		...
		try {
			in=new BufferedReader(new FileReader(Category.FILE_NAME));
			out=new PrintWriter(new FileWriter(Category.TMP_FILE_NAME));
		} catch (FileNotFoundException e) {
			// TODO: handle exception
			System.out.println("메뉴 파일을 찾을 수 없습니다.");
			return;
		...
		try {
			while((s=in.readLine())!=null) {//Category.FILE_NAME (카테고리파일)을 읽어올동안
				category=s.split("\\|");
			
				//해당 카테고리ID를 가진 행이 나타나면
				if(category[Category.INDEX_CATEGORY_ID].equals(newCategoryData[Category.INDEX_CATEGORY_ID])) {
				...
				if (category[Category.INDEX_NAME].equals(newCategoryData[Category.INDEX_NAME])) {
		...
		File file=new File(Category.FILE_NAME);
        file.delete();

        file=new File(Category.TMP_FILE_NAME);
        file.renameTo(new File(Category.FILE_NAME));
	}
	...
	public void toggleCategoryEnable(String toggleCategoryId) {
	    ...
	    try {
	        ...
	        while ((s=in.readLine())!=null) {
	            ...
	            if (toggleCategoryId.equals(category[Category.INDEX_CATEGORY_ID])) {//활성화 여부를 변경하고자 하는 카테고리를 찾으면 
	                categoryFound=true;
	                if (category[Category.INDEX_ENABLE].equals("Y")) {
						category[Category.INDEX_ENABLE]="N"; //카테고리의 활성화 여부 반대로 변경
					}else category[Category.INDEX_ENABLE]="Y";
	        ...
	        File categoryFile=new File(Category.FILE_NAME);
	        categoryFile.delete();

	        File tmpCategoryFile=new File(Category.TMP_FILE_NAME);
	        tmpCategoryFile.renameTo(new File(Category.FILE_NAME));
